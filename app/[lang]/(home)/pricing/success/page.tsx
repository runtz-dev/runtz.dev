...
export default function PricingSuccessPage() {
  ...
  const copy = pricingCopy[locale].success;
  ...
  useEffect(() => {
    ...
    fetch(`${apiURL}/api/v1/billing/checkout-session/${encodeURIComponent(sessionID)}`)
      .then(async (response) => {
        ...
        if (!response.ok) {
          throw new Error(typeof payload.error === 'string' ? payload.error : copy.loadError);
        }
        setStatus(payload as CheckoutStatus);
      })
      .catch((error) => setError(error instanceof Error ? error.message : copy.loadError));
  }, []);
            ...
            <div className="mt-8 grid gap-5">
              <div className="grid gap-3 rounded-2xl border border-[#071222]/10 bg-white/45 p-4 text-sm dark:border-[#6db5ff]/12 dark:bg-white/5">
                <div className="flex justify-between gap-4">
                  <span className="text-[#53657d] dark:text-[#9fb4cf]">{copy.planLabel}</span>
                  <span className="font-semibold capitalize">{status.plan}</span>
                </div>
                <div className="flex justify-between gap-4">
                  <span className="text-[#53657d] dark:text-[#9fb4cf]">{copy.deploymentLabel}</span>
                  <span className="font-semibold">{status.deploymentMode}</span>
                </div>
                <div className="flex justify-between gap-4">
                  <span className="text-[#53657d] dark:text-[#9fb4cf]">{copy.statusLabel}</span>
                ...
                <div className="rounded-2xl border border-[#2f7eff]/25 bg-[#eaf4ff] p-4 dark:border-[#6db5ff]/20 dark:bg-[#101827]">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <h2 className="font-semibold">{copy.fallbackTitle}</h2>
                      {status.licenseKey ? (
                        <>
                          ...
                          >
                            ...
                            {copied ? copy.copied : copy.copyKey}
                          </button>
                        </>
                      ) : (
                        <p className="mt-2 text-sm leading-6 text-[#53657d] dark:text-[#b8cbe4]">
                          {copy.keyIssued}{' '}
                          <span className="font-mono">{status.licenseKeyPrefix ?? copy.unknown}</span>.
                        </p>
                      )}
                    </div>
                  </div>
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                ...
                <SecondaryLink
                  href={`${localizedPath(locale, '/docs/docker-compose')}#self-hosted-pro-and-enterprise-activation`}
                >
                  {copy.activationDocs}
                </SecondaryLink>
              </div>
