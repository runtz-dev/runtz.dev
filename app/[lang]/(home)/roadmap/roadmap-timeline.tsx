...
import type { RoadmapCopy } from '@/lib/roadmap-copy';
...
export function RoadmapTimeline({ copy }: { copy: RoadmapCopy }) {
  const milestones = copy.milestones;

  return (
    <div className="rz-roadmap-frame rounded-2xl border px-5 py-10 sm:px-7 md:px-12 md:py-12">
      <h2 className="sr-only">{copy.timelineLabel}</h2>
      <Timeline
        defaultValue={4}
        className="mx-auto w-full max-w-3xl"
        render={<ol aria-label={copy.timelineLabel} />}
      >
          ...
          >
            <TimelineHeader>
              ...
              >
                ...
                <StatusBadge status={milestone.status} copy={copy.status} />
              </div>
        ...
        >
          <TimelineHeader>
            <TimelineSeparator className="rz-roadmap-separator" />
            <TimelineTitle className="sr-only">{copy.backlog}</TimelineTitle>
            <TimelineIndicator className="rz-roadmap-indicator rz-roadmap-indicator-backlog">
              <span>{copy.backlog}</span>
            </TimelineIndicator>
          </TimelineHeader>
        </TimelineItem>

        {copy.backlogItems.map((title, index) => {
