...
A CLI do runtz é um único binário estático em Go. O script detecta o sistema operacional e a arquitetura, baixa o binário e o instala em `/usr/local/bin`:
...
Linux e macOS são compatíveis com `amd64` e `arm64`.

### Opções do script

O script aceita variáveis de ambiente:
