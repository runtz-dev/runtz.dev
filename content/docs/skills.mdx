...
1. **Run scans with the runtz CLI.** Every scan needs the CLI installed and a
   token — from a one-time `runtz login`, or `--token`/`RUNTZ_TOKEN` (and
   `--endpoint`/`RUNTZ_ENDPOINT` when self-hosted).
