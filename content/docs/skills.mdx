...
1. **Prefer the [runtz MCP server](/docs/mcp)** when it is connected — the
   assistant calls `runtz_sca`, `runtz_sast`, `runtz_host`, `runtz_container`,
   `runtz_k8s`, and the `runtz_docs_*` tools.
...
Pair any skill with the [runtz MCP server](/docs/mcp) for the best experience.
