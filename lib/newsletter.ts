...
export const getNewsletterTags = cache(async () =>
  (await json<{ items: NewsletterTag[] }>('/v1/tags')).items,
);

export const getNewsletterArticle = cache(async (slug: string, locale: string): Promise<NewsletterArticle | null> => {
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(slug)) return null;
  const path = `/v1/posts/${encodeURIComponent(slug)}`;
  let response = await newsletterFetch(`${path}?locale=${encodeURIComponent(locale)}`);
  // A locale without a translation redirects to the article's actual language.
  if (response.status === 404) response = await newsletterFetch(path);
  if (response.status === 404) return null;
  if (!response.ok) throw new NewsletterUnavailable();
  return response.json() as Promise<NewsletterArticle>;
});

export function articlePath(post: Pick<NewsletterPost, 'locale' | 'slug'>) {
  return localizedPath(post.locale, `/newsletter/${post.slug}`);
}
